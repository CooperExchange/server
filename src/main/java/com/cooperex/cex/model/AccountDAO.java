...
public class AccountDAO extends DataAccessObject {
    ...
    public String addBalanceById(String userId, Double amount) {
        String ADD_BALANCE_BY_ID = "update accounts set total_deposit = total_deposit + ? where user_id = ?;";
        int userIdInt = Integer.parseInt(userId);

        try (PreparedStatement statement = this.connection.prepareStatement(ADD_BALANCE_BY_ID);) {
    ...
    public String withdrawBalanceById(String userId, Double amount) {
        String UPDATE_WITHDRAWAL_BALANCE_BY_ID = "update accounts set total_withdrawal = total_withdrawal + ? where user_id = ?;";
        int userIdInt = Integer.parseInt(userId);
        try (PreparedStatement statement = this.connection.prepareStatement(UPDATE_WITHDRAWAL_BALANCE_BY_ID);) {
            statement.setDouble(1, amount);
            statement.setInt(2, userIdInt);
            statement.executeUpdate();
    ...
    public String tradeAssetBySymbol(String userId, Trade trade) {
        ...
        Double assetCountDoucle = Double.parseDouble(trade.getAssetCount());
        ...
        try {
            ...
            statement.setString(1, trade.getTradeType());
            statement.setInt(2, userIdInt);
